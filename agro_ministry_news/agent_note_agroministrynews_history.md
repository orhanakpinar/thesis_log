# Process history: agro_ministry_news (AI-authored working note)

**This is the dated historical record for the `agro_ministry_news/` strand. It was split out
of `agent_note_agroministrynews_FSOI.md` on 2026-09-21** to keep the working note loadable —
the combined file had reached ~1,660 lines, and a fresh session had to read all of it to find
the parts that were still true.

**Read the working note first.** `agent_note_agroministrynews_FSOI.md` holds current state
(Part 1) and stable reference (Part 2), and is current wherever it disagrees with anything
here. This file is the record of *how* the strand got there.

**Naming note:** this file says `Buyuksehir_Law` throughout. That category was renamed to
**`Metropolitan_Law`** on 2026-09-22 (in this strand only — `literature_research/` still uses
the old name, deliberately, until Orhan renames it there). **The old name is left intact below
on purpose:** these are dated records of runs that genuinely used it, and rewriting them would
misrepresent what those batches actually produced. Read `Buyuksehir_Law` here as
`Metropolitan_Law` today.

**Nothing here was deleted in the split — the content below is verbatim.** It is kept because
several sections are substantive findings rather than process log, and Orhan asked that the
methodology narrative survive intact for the thesis write-up: the `zeyrek` mis-lemmatization
bugs, the Haber/392 Metropolitan Law saga, the category-scheme evolution, and the
rule-based-NLP-tried-and-abandoned story.
...
## Moved from the working note, 2026-09-21

*Trimmed out of `agent_note_agroministrynews_FSOI.md` to keep it loadable. Nothing was
deleted — the full original text is below. The working note keeps the durable facts from each
of these and points here for the reasoning.*


### Full-text scraping status (2026-09-11 narrative)
...
### 44-category codebook list (third copy)
...
### Standardized-prompt meta-section (pre-snapshot argument)
...
### Handoff status (2026-09-19 detail)
