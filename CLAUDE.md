...
**Sharpen this further — the silence is specifically the STATE's, not universal (found
2026-09-24).** TMMOB Ziraat Mühendisleri Odası (Chamber of Agricultural Engineers — a
professional civil-society body, not a state actor) explicitly connects Law 6360 to
agriculture in its 48th General Assembly result declaration: village assets were
transferred to municipal control and redirected away from agricultural purposes
("köy tüzel kişiliğinin varlıkları ilgili belediyenin tasarrufuna verilerek"), village
populations have aged as a consequence, and the declaration calls for the law to be
reviewed for efficient rural-support resource allocation
(https://www.zmo.org.tr/icerik/tmmob-ziraat-muhendisleri-odasi-48-donem-olagan-genel-kurulu-sonuc-bildirgesi-yayinlandi-31893).
**This does not weaken the finding — it sharpens it into a real contrast worth stating
explicitly:** the state's own legislative/administrative apparatus (Gazette, ministry press)
essentially never frames 6360 as agricultural policy, **while civil-society professional
bodies explicitly do, and criticize the law on exactly those grounds.** That contrast — official
silence vs. professional-body critique — is itself evidence for the theoretical reading
above (the state frames 6360 as administrative reform; the agricultural reading is an
outside interpretation, here coming from a professional body rather than only from
academia) and is a legitimate citation for the discussion chapter, not a competing finding
to reconcile away.
...
aggregation-dependent." **Refinement (2026-09-24), doesn't contradict this:** descriptive
LOCO extended to all 5 Track C categories shows non-metro leads regardless of *which single
category* is dropped — so "non-metro leads" is robust to category composition, just not to
changing the aggregation method entirely. Two different axes of robustness: stable under
category removal, unstable under aggregation-method change. This does **not** touch the causal DiD result — the DiD's control
...
- **Bottom line, across all four specifications now run (category-weighted primary,
  ...
  and more seriously: see CORRECTION 2 (2026-09-24) — whether the `municipal_burden` effect
  itself is real or a denominator artifact is unresolved.**
...
entirely.** (This combined-category null is real, but see CORRECTION 1 below — dated
2026-09-24 — for why it's not the same as "production, land-use and external-input show
...
- **Defensible from this data:** "Law 6360 changed waste-per-household outcomes in treated
  cities."
- **NOT defensible from this data:** "Law 6360 reduced food sovereignty." Production shows
  nothing; land-use and external-input show small, real, opposite-signed effects that net
  to roughly zero combined (CORRECTION 1 below, 2026-09-24) — none of this supports a
  food-sovereignty claim, but "show nothing" overstates it for two of the three.
...
pure boundary-expansion**, since boundary-expansion alone gives no reason for two separate
variables to fade in sync with a date tied to the waiver's own end, rather than just any
date. Still not a certainty (the COVID-year overlap at 2020 still can't be fully separated
out), but this moves the needle from "not distinguishable" toward "waiver mechanism looks
more likely," on the variable it was actually aimed at.
...
- **LOCO under flat weighting: −0.0082, p=0.174 — not significant.** Sign stays negative
  ...
  still disappears. So the *combined* null on the remaining three categories now holds
  under **two independently-built weighting schemes**, not one — though see CORRECTION 1
  below (2026-09-24) for why "combined null" isn't the same claim as "each is null."
...
**So the picture WAS internally consistent as of 2026-09-23 — see two corrections below,
found 2026-09-24, before treating any of this as settled.** The −0.037 effect is
statistically solid — it survives the correction built to catch exactly this failure mode.
"It is entirely generated by `municipal_burden`, with no residual signal in
production/land-use/external-input" — **this specific sentence is corrected below; don't
cite it as written.**

⚠️ **CORRECTION 1 (2026-09-24) — "no detectable effect on production/land-use/external-
input" was overstated. Only production is genuinely null.** Full LOCO plus per-category
isolation (each category run alone as a single-category composite) found:
- **production: genuinely null** (the only one of the three that truly shows nothing).
- **land-use: −0.023, p=0.003 — significant, same direction as `municipal_burden` (worse
  for treated).**
- **external-input: +0.048, p=0.004 — significant, OPPOSITE direction (favorable for
  treated).**
- **`municipal_burden`: −0.175, p<0.0001 — still an order of magnitude larger than either.**
Land-use and external-input's effects are real but small, and **roughly cancel each other**
when averaged into the combined 3-category LOCO composite — which is *why* that combined
test looked like a clean null. It wasn't "nothing happening"; it was two small, opposite,
real effects offsetting. **`municipal_burden` remains clearly the dominant driver by an
order of magnitude — this does not overturn the headline conclusion** — but "agriculture
shows nothing" must become "production shows nothing; land-use and external-input show
small, real, opposite-signed effects that net to roughly zero when combined." That's a more
precise and more interesting claim, not a weaker one. Corrected everywhere this was stated
...
— including via the same municipal-boundary-redefinition mechanism already documented as a
confound (see the Law 6360 confounds above)** — the effect is **−0.0003, p=0.949: nothing at
all, even with `municipal_burden` included.** So the track built to be immune to exactly the
kind of denominator drift Law 6360 causes shows **no effect whatsoever**, while the track
vulnerable to that drift shows a significant one. This is not a robustness check that failed
quietly — it's a live, open question whether the entire `municipal_burden` finding is a real
economic effect or a household-count denominator artifact of the same reform. **Not
resolved. Do not present the −0.037 finding as settled until Orhan has weighed in on this
...
as post-treatment ones, just smaller — reading as an ordinary secular urban-growth trend,
not a reform-triggered discontinuity. `Mean_Household_Size` shows no effect anywhere
(p>0.15 throughout). **This specifically rules out the discontinuous-denominator-jump
version of the artifact hypothesis** (the same signature already confirmed for the water
measurement confound). **It does NOT resolve the broader perArea-vs-perHousehold tension —
...
groups, not *differential trends*. Lower-grade than the jump hypothesis, but real and
